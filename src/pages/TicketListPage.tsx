...
export function TicketListPage({
  ...
  const filtered = [...tickets]
    ...
    .filter((ticket) => {
      ...
      return [ticket.no, ticket.hn, ticket.name].some((value) =>
  ...
  return (
    <div className="animate-fade-up">
      <div className="mb-[20px] flex flex-wrap items-end justify-between gap-[14px]">
        <div><h1 className="mb-[4px] text-[clamp(20px,3vw,26px)] font-bold text-[#0f172a]">รายการใบค้างยา</h1><p className="text-[13.5px] text-[#64748b]">พบ {filtered.length} รายการ</p></div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-[18px] border border-[#e7eef7] bg-white px-[24px] py-[56px] text-center">
          ...
          <button onClick={() => { onQueryChange(''); onStatusChange('all'); }} className="cursor-pointer rounded-[11px] border border-[#bfdbfe] bg-[#eff6ff] px-[20px] py-[10px] text-[13.5px] font-bold text-[#1d4ed8] hover:bg-[#dbeafe]">ล้างตัวกรองทั้งหมด</button>
        </div>
      ) : (
        <div className="overflow-hidden rounded-[18px] border border-[#e7eef7] bg-white">
          <TicketRows tickets={filtered} mode="list" onView={onView} onPrint={onPrint} />
        </div>
      )}
    </div>
  );
