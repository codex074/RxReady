  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[linear-gradient(180deg,#eef4fb_0%,#dbe9fb_100%)] px-[18px] py-[36px]">
      ...
      <div className="animate-fade-up-slow w-full max-w-[440px] rounded-[22px] border border-[#e2e8f0] bg-white px-[28px] py-[30px] shadow-[0_20px_46px_-28px_rgba(15,42,90,.45)]">
        ...
        <p className="mb-[24px] text-center text-[14px] leading-[1.6] text-[#64748b]">กรอกวันที่รับบริการและเบอร์โทร 4 หลักท้าย<br />เพื่อตรวจสอบสถานะยาของท่าน</p>
        <form onSubmit={onSubmit}>
          <label className="mb-[7px] block text-[14px] font-semibold text-[#334155]">วันที่รับบริการ</label>
          <input
            type="date"
            value={visitDate}
            onChange={(event) => onVisitDateChange(event.target.value)}
            max={new Date().toISOString().slice(0, 10)}
            className="mb-[18px] w-full rounded-[13px] border border-[#cbd5e1] px-[15px] py-[14px] text-[16px] text-[#0f172a] outline-none focus:border-[#2563eb] focus:shadow-[0_0_0_3px_rgba(37,99,235,.16)]"
            required
          />
          <label className="mb-[7px] block text-[14px] font-semibold text-[#334155]">เบอร์โทร 4 หลักท้าย</label>
          <input
            value={phoneLast4}
            onChange={(event) => onPhoneLast4Change(event.target.value.replace(/\D/g, '').slice(0, 4))}
            placeholder="เช่น 4567"
            inputMode="numeric"
            maxLength={4}
            className="mb-[24px] w-full rounded-[13px] border border-[#cbd5e1] px-[15px] py-[14px] text-[16px] tracking-[.2em] tabular-nums text-[#0f172a] outline-none focus:border-[#2563eb] focus:shadow-[0_0_0_3px_rgba(37,99,235,.16)]"
            required
          />
          <button
            type="submit"
            disabled={loading}
            className="flex w-full cursor-pointer items-center justify-center gap-[9px] rounded-[13px] border-0 bg-[#2563eb] p-[15px] text-[16px] font-bold text-white disabled:opacity-60 hover:bg-[#1d4ed8]"
          >
